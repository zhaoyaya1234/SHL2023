...
class ParseMMTraj(ParseTraj):
    ...
    def parse(self, all_traj):
        ...
        for single_traj in tqdm(content_list):
            ...
            tid = tid_list[i]
            # 单点轨迹特征
            pt_list = single_traj.apply(lambda x:STPoint(x.latitude,x.longitude,datetime.strptime(str(x.timestamp),time_format), {'candi_pt':CandidatePoint(x.proj_lat, x.proj_lng, x.eid, x.error, x.offset, x.rate),'bearing':x.bearing,'traj_features':[x.speed,x.speed_gnss,x.speed_hypot,x.speed_hypot_sub_gnss,x.accel_lateral,x.accel_lateral_gnss,x.accel_lateral_hypot,x.accel_longitudinal,x.accel_longitudinal_gnss,x.accel_longitudinal_hypot]}), axis=1)
            pt_list = [pt_list[i] for i in range(len(pt_list)) ]
            traj = Trajectory(oid, tid, pt_list)
            # print('dist',traj.get_distance()) 
            trajs.append(traj)
            i+=1
        return trajs
