...
def split_data(traj_input_dir, output_dir):
    ...
    traj_input_path = os.path.join(traj_input_dir,'traj_dict_map.npy')
...
class Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, trajs_dir, mbr,online_features_dict,\
        ...
        self.src_grid_seqs, self.src_gps_seqs, self.src_pro_fea,self.src_point_fea,self.src_rid,self.src_rate,self.src_road_fea = [], [], [],[],[],[],[]
        self.trg_gps_seqs, self.trg_rids, self.trg_rates = [], [], []
        self.new_tids = []
        # above should be [num_seq, len_seq(unpadded)]
        random.seed(seed_value)
        self.get_data(trajs_dir,online_features_dict, parameters.win_size, 
    ...
    def __getitem__(self, index):
        ...
        return src_grid_seq, src_gps_seq, src_pro_fea,src_point_fea,src_rid,src_rate,src_road_fea, trg_gps_seq, trg_rid, trg_rate
    ...
    def add_token(self, sequence):
        ...
        return new_sequence
    def get_data(self,trajs_dir,online_features_dict, win_size, \
                      ds_type, keep_ratio):
        # parser = ParseMMTraj()
        ...
        for traj in tqdm(trajs):
            ...
            ls_grid_seq_ls, ls_gps_seq_ls, features_ls,point_features_ls,ls_rid_ls,ls_src_rate,ls_rid_features = self.parse_traj(traj,online_features_dict,win_size, ds_type, keep_ratio)
        ...
        assert len(self.new_tids) == len(self.trg_gps_seqs) == len(self.trg_rids) == len(self.trg_rates) == \
               len(self.src_gps_seqs) == len(self.src_grid_seqs) == len(self.src_pro_fea) == len(self.src_point_fea)== len(self.src_rid) == len(self.src_road_fea), \
        'The number of source and target sequence must be equal.'

    def parse_traj(self,traj,online_features_dict, win_size, ds_type, keep_ratio):
        """
        ...
        ls_grid_seq_ls, ls_gps_seq_ls, features_ls,point_features_ls,ls_rid_ls,ls_src_rate,ls_rid_features  = [], [], [],[],[],[],[]
        ...
        for tr in new_trajs:
            ...
            ls_grid_seq, ls_gps_seq,point_features, ls_src_eids,ls_src_rate_s,rid_features,hours, ttl_t = self.get_src_seq(ds_pt_list,online_features_dict)
    ...
    def get_src_seq(self, ds_pt_list,online_features_dict):
        hours = []
        ls_grid_seq = []
        ls_gps_seq = []
        point_features = []
        ...
        for ds_pt in ds_pt_list: 
            ...
            rid_features.append(online_features_dict[ds_pt.data['candi_pt'].eid])
            
            
        return ls_grid_seq, ls_gps_seq,point_features, ls_src_eids,ls_src_rate_s,rid_features,hours, ttl_t # 每个点对应的小时，总时间
...
def collate_fn(data,rn, args):
    ...
    src_grid_seqs, src_gps_seqs, src_pro_feas,src_point_feas,src_rids,src_rates,src_road_feas, trg_gps_seqs, trg_rids, trg_rates =zip(*data)
    ...
    b_size = constraint_mat.shape[0]
    for b in range(b_size):
        trg_len= trg_lengths[b]
        mask = (constraint_mat[b][:trg_len] == 1).sum(axis=1) >= 1
        indices = torch.arange(trg_len)[mask]
        for i in indices:
            if constraint_mat[b][i][trg_rids[b][i]]!=1:
                constraint_mat[b][i][trg_rids[b][i]] =1

    return src_grid_seqs, src_gps_seqs, src_pro_feas,src_point_feas,src_rids,src_rates,src_road_feas, src_lengths, trg_gps_seqs, trg_rids, trg_rates, trg_lengths,constraint_mat
