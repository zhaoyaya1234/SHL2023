...
class SHLDataLoader():
    ...
    def load_gps(self, detail = True):
        ...
        print("GPS 读取完成，共 {} 条数据，用时 {}s".format(self.gps.shape[0], timer.get_update_time()))
        if detail:
            ...
            print("\t-- GPS 详细信息提取完成，共 {} 条数据，用时 {}s".format(self.gps_detail.shape[0], timer.get_update_time()))
    ...
    def load_mag(self):
        print("mag Loading...")
        timer = TimeKeeper()
        loc_names = ['time', 'MAG_x', 'MAG_y', 'MAG_z']
        self.mag = pd.read_table(self.root_path + '/Mag.txt', header = None, names = loc_names, sep = "\t")
        self.mag = self.mag.iloc[:int(self.load_ratio * self.mag.shape[0]),:] if self.load_ratio else self.mag
        print("Mag 读取完成，共 {} 条数据，用时 {}s".format(self.mag.shape[0], timer.get_update_time()))
    
    def load_gyr(self):
        print("Gyr Loading...")
        timer = TimeKeeper()
        loc_names = ['time', 'GYR_x', 'GYR_y', 'GYR_z']
        self.gyr= pd.read_table(self.root_path + '/Gyr.txt', header = None, names = loc_names, sep = "\t")
        self.gyr = self.gyr.iloc[:int(self.load_ratio * self.gyr.shape[0]),:] if self.load_ratio else self.gyr
        print("Gyr 读取完成，共 {} 条数据，用时 {}s".format(self.gyr.shape[0], timer.get_update_time()))
    
    def load_acc(self):
        print("Acc Loading...")
        timer = TimeKeeper()
        loc_names = ['time', 'ACC_x', 'ACC_y', 'ACC_z']
        self.acc = pd.read_table(self.root_path + '/Acc.txt', header = None, names = loc_names, sep = "\t")
        self.acc = self.acc.iloc[:int(self.load_ratio * self.acc.shape[0]),:] if self.load_ratio else self.acc
        print("Acc 读取完成，共 {} 条数据，用时 {}s".format(self.acc.shape[0], timer.get_update_time()))
